...
describe("UserController", () => {
	...
	it.only("should return user count", async () => {
	...
	// 	await expect(userController.one({params: {id: "1"}} as any)).rejects.toThrow(
	// 		"unregistered user",
	// 	);
	// });
	//
	...
	// 	mockRepository.findOne = jest.fn().mockResolvedValue(undefined);
	// 	await expect(
	// 		userController.update({params: {id: "1"}, body: undefined} as any),
	// 	).rejects.toThrow("this user not exist");
	// });
	//
	...
	// 	mockRepository.findOne = jest.fn().mockResolvedValue(undefined);
	// 	await expect(userController.remove({params: {id: "1"}} as any)).rejects.toThrow(
	// 		"this user not exist",
	// 	);
	// });
});
